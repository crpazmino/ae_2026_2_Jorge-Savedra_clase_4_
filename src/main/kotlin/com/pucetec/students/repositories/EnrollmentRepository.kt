...
// JpaRepository<Enrollment, Long> recibe dos tipos:
// ┌─────────────────────────────────────────────┐
// │ Enrollment → la entidad que maneja          │
// │ Long       → el tipo del ID de la entidad   │
// └─────────────────────────────────────────────┘

// Métodos que Spring genera automáticamente sin escribir SQL:
// ┌────────────────────────────────────────────────────────┐
// │ save(enrollment)        → INSERT o UPDATE en la BD     │
// │ findById(id)            → SELECT WHERE id = ?          │
// │ findAll()               → SELECT * FROM enrollment     │
// │ deleteById(id)          → DELETE WHERE id = ?          │
// │ existsById(id)          → SELECT COUNT WHERE id = ?    │
// │ count()                 → SELECT COUNT(*) FROM enrollment│
// └────────────────────────────────────────────────────────┘